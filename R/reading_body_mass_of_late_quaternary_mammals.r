...
reading_body_mass_of_late_quaternary_mammals <- function() {
   ...
   if (!file.exists('./data/downloaded_data/traits/99_bird_species_data-02-00012-s001/Table 1.csv')) {
      ...
      unzip(zipfile = './data/cache/traits/body_mass_of_late_quaternary_mammals.zip',
            exdir = './data/downloaded_data/traits/body_mass_of_late_quaternary_mammals')
      file.remove('./data/cache/traits/body_mass_of_late_quaternary_mammals.zip')
   }

   # reading the csv
   mom <- data.table::fread('./data/downloaded_data/traits/body_mass_of_late_quaternary_mammals/MOMv3.3.txt', sep = '\t', dec = '.', header = FALSE, na.strings = '-999')
