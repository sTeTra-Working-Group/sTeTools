...
reading_rate_of_living_in_tetrapods_data <- function() {
   create_folders()
   if (!file.exists( './data/downloaded_data/traits/rate_of_living_in_tetrapods.csv')) {
      download.file(
      ...
      data.table::fwrite(x = rol, file = './data/downloaded_data/traits/rate_of_living_in_tetrapods.csv', dec = '.', sep = ',')

      file.remove('./data/cache//traits/rate-of-living_geb13069-sup-0001-AppendixS1.xlsx')
      return(rol)
   } else {
      return(
         data.table::fread(file = './data/downloaded_data/traits/rate_of_living_in_tetrapods.csv',
                           dec = '.', sep = ',', header = TRUE)
      )
   }
}
